...
package waitgroupext_test

import (
	. "github.com/abligh/waitgroupext"
	"runtime"
	"sync/atomic"
	"testing"
)
...
func TestWaitGroupMisuse(t *testing.T) {
	defer func() {
		err := recover()
		if err != "waitgroupext: negative WaitGroup counter" {
...
func TestWaitGroupMisuse2(t *testing.T) {
	...
	defer func() {
		err := recover()
		if err != "waitgroupext: negative WaitGroup counter" &&
			err != "waitgroupext: WaitGroup misuse: Add called concurrently with Wait" &&
			err != "waitgroupext: WaitGroup is reused before previous Wait has returned" {
			t.Fatalf("Unexpected panic: %#v", err)
		}
	}()
